...
//connection returns a promise 
    .then(client => {
        //log a message to the console to  confirm the connection worked
        console.log(`Connected to ${dbName} Database`)
        //chain the promise to the db method using the variable assigned to the db name
        db = client.db(dbName)
    })
//set up middleware
//this MW is used to set up the ejs template  
app.set('view engine', 'ejs')
//this MW is used to apply the public files
app.use(express.static('public'))
//this is used to parse the body of a request
...
//This is the create or post method used to input stuff into the DB using a form, url or anchor tag
app.post('/addTodo', (request, response) => {
    //a request is made to the server to go tothe db and then to the collection to inset a document matching a specific criteria
    db.collection('todos').insertOne({thing: request.body.todoItem, completed: false})
    //a promise is returned 
    .then(result => {
        // a message is logged to the console to confirm that the request is successful
        console.log('Todo Added')
        //a response to redirecto to the root is sent back
        response.redirect('/')
    })
...
//This is a put or update method used to update documents in the database
app.put('/markComplete', (request, response) => {
    ...
    db.collection('todos').updateOne({thing: request.body.itemFromJS},{
        ...
        sort: {_id: -1},
        //the upsert method is used to update and insert the value of a field when the field we are currently updating can no longer be updated
        upsert: false
    })
    // a promise is returned
    .then(result => {
...
//this method is used to delete documents in the Db 
